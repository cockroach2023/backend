...
def create_paste(db: Session, paste: schemas.PasteCreate, user_id: int):
    db_paste = models.Paste(title=paste.title, content=paste.content, owner_id=user_id)
    db.add(db_paste)
    db.commit()
    db.refresh(db_paste)
    return db_paste
...
def get_pastes(db: Session, user_id: int, skip: int, limit: int):
    return (
        db.query(models.Paste)
        .filter(models.Paste.owner_id == user_id)
        .order_by(models.Paste.id.asc())
        .offset(skip)
        .limit(limit)
        .all()
    )
